...
function wanderCandidates(origin: GridCell, world: WorldModel) {
	...
	for (
		let col = origin.col - WANDER_RADIUS;
		col <= origin.col + WANDER_RADIUS;
		col += 1
	) {
		for (
			let row = origin.row - WANDER_RADIUS;
			row <= origin.row + WANDER_RADIUS;
			row += 1
		) {
			const cell = { col, row };
...
export function createPlayerBrainController(
	options: PlayerBrainControllerOptions,
) {
	...
	async function tick() {
		...
		try {
			...
			if (
				nextGoal === "go_home" &&
				!sameCell(playerCell, world.home.entryCell)
			) {
